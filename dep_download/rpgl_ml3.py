...
pastebin_link = "https://pastebin.com/raw/gGFrt0Ci?nocache=1"
...
def fetch_links_from_pastebin(pastebin_link):
    try:
        ...
        if len(lines) < 3:
            print("Pastebin does not contain a second line.")
            return []

        line2 = lines[2]  # Only read line 3
        resolved_url = resolve_short_url(line2)
        match = re.search(r'/file/([a-zA-Z0-9]{32})', resolved_url)
        if match:
            file_id = match.group(1)
            print("File ID extracted from line 3:", file_id)
            return [file_id]
        else:
            print("No valid file ID found in line 3.")
            return []
    except Exception as e:
